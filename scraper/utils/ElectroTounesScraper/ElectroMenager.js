...
(async () => {
  ...
  await page.goto('https://electrotounes.tn/electromenager?page=1', {
    ...
    for (const productHandle of productsHandles) {
      try {
        ...
        const price = await productHandle.$eval(
            ...
            const reference = referenceElement ? await referenceElement.evaluate(el => el.nextSibling.textContent.trim()) : 'Reference not found';
        ...
        const inStockElement = await productHandle.$(".info-stock > .in-stock > .instock");
        const laterStockElement = await productHandle.$(".info-stock > .in-stock > .outofstock");
        ...
        newData.push({ title: 'Error' , price:'Error' ,reference: 'Error' ,description: 'Error' , availability: 'Error' ,img:'Error' ,productUrl : 'Error' });
      }
    }

...
(async () => {
  ...
  await page.goto('https://www.mytek.tn/telephonie-tunisie/smartphone-mobile-tunisie.html?p=1', {
    waitUntil: "domcontentloaded",
  });

   // Function to get the class of the last page
   const getLastPageNumber = async () => {
    const lastPageElement = await page.$('.pages > ul.items.pages-items > li.page.last > span:last-child'); // Select the last span inside the li.page.last element
    return lastPageElement ? await lastPageElement.evaluate(el => el.textContent.trim()) : null;
};
  ...
  while (true) {
    ...
    const productsHandles = await page.$$('.item.product.product-item > .product-item-info');
    for (const productHandle of productsHandles) {
      try {
        // Extract title, price, description, availability, and image
        // const title = await productHandle.$eval(".product.details.product-item-details > strong.product.name.product-item-name > a ", (el) => el.textContent.trim());
        const titleElement = await productHandle.$(".prdtBILDetails > .product.details.product-item-details > strong.product.name.product-item-name > a");
        const title = titleElement ? await titleElement.evaluate(el => el.textContent.trim()) : 'Title not found';
         ...
         let price = 0; // Initialize price as a number with a default value
          ...
          const hasNewPrice = await productHandle.$(".price-box.price-final_price > span.special-price > span.price-container.price-final_price.tax > span.price-wrapper > span.price");
          const defaultPrice = await productHandle.$(".price-box.price-final_price > span.price-container.price-final_price.tax > span.price-wrapper > span.price");
        ...
        const inStockElement = await productHandle.$(".card.text-white.bg-secondary.mb-3 > .card-body > .stock.available > span");
        const laterStockElement = await productHandle.$(".card.text-white.bg-secondary.mb-3 > .card-body > .stock.unavailable > span");
